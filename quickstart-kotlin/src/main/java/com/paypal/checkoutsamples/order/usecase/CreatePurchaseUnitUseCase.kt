...
class CreatePurchaseUnitUseCase(
    ...
    fun execute(request: CreatePurchaseUnitRequest): PurchaseUnit = with(request) {
        ...
        return PurchaseUnit.Builder()
            ...
            .amount(CreateAmountUseCase().execute(CreateAmountRequest(createdItems, currencyCode)))
            .items(CreateItemsUseCase().execute(CreateItemsRequest(createdItems, currencyCode)))
            /*
             * Omitting shipping will default to the customer's default shipping address.
             */
            .shipping(CreateShippingUseCase().execute(CreateShippingRequest(shippingPreference, currencyCode)))
            /*
